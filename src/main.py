...
def text_messages(message):
    if message.text == "Ответы на часто задаваемые вопросы":
        ...
        bot.send_message(whilelse_chat_id,
                         f"Вопрос от {message.from_user.first_name} (@{message.from_user.username}): {message.text}")
        bot.send_message(message.chat.id, "Ваш вопрос был отправлен. Мы свяжемся с вами позже.")

def show_questions_menu(message):
    keyboard = types.InlineKeyboardMarkup()
    question_one = types.InlineKeyboardButton(text='Вопрос 1', callback_data='question_1')
    question_two = types.InlineKeyboardButton(text='Вопрос 2', callback_data='question_2')
    question_three = types.InlineKeyboardButton(text='Вопрос 3', callback_data='question_3')
    question_four = types.InlineKeyboardButton(text='Вопрос 4', callback_data='question_4')
    question_five = types.InlineKeyboardButton(text='Вопрос 5', callback_data='question_5')
    question_six = types.InlineKeyboardButton(text='Вопрос 6', callback_data='question_6')
    back_button = types.InlineKeyboardButton(text='Вернуться назад', callback_data='back_to_main')

    keyboard.add(question_one, question_two, question_three, question_four, question_five, question_six, back_button)
...
def callback_query(call):
    ...
    if call.data == 'question_1':
        ...
        btn1 = types.InlineKeyboardButton("Вопрос 1 по теме", callback_data="option1")
        btn2 = types.InlineKeyboardButton("Вопрос 2 по теме", callback_data="option2")
